...
class WatchZone(Base):
    ...
    geometry = Column(Geometry(geometry_type='POLYGON', srid=4326) if 'sqlite' not in DATABASE_URL else String)
...
class SatelliteMetadata(Base):
    ...
    aoi_geometry = Column(Geometry(geometry_type='POLYGON', srid=4326) if 'sqlite' not in DATABASE_URL else String)
